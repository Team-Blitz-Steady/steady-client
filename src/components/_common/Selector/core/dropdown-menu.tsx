...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ref={ref}
    className={cn(
      "flex cursor-default select-none items-center rounded-sm px-8 py-6 text-sm outline-none focus:bg-accent data-[state=open]:bg-accent",
      inset && "pl-32",
      className,
    )}
    {...props}
  >
    {children}
    <ChevronRightIcon className="ml-auto h-16 w-16" />
  </DropdownMenuPrimitive.SubTrigger>
));
...
>(({ className, children, ...props }, ref) => (
  ...
  >
    <span className="absolute left-8 flex h-14 w-14 items-center justify-center">
      <DropdownMenuPrimitive.ItemIndicator>
        <CheckIcon className="fill-current h-8 w-8" />
      </DropdownMenuPrimitive.ItemIndicator>
    </span>
    {children}
  </DropdownMenuPrimitive.RadioItem>
